...
$feeMaster = $conn->query("select * from fees_master where status=1  order by id ASC");
   ...
   while($fM = $feeMaster->fetch_assoc()){
    //echo "<pre>";
       ...
       $data[]=$fM;
        ...
        getCopyTable($conn,$data,$created_date,$modify_date,$count,$check_status);
   }
  
   

}
function getCopyTable($conn,$data,$created_date,$modify_date,$count,$check_status){
    //  echo "<pre>";
    // print_r($sold_status);
    // $sold_status =[];
    foreach($data as $key =>$datas){
        ...
        if(mysqli_num_rows($peti)> 0){
          ...
          //print_r($fee_count);
            ...
            $parent_id = $datas['id'];
            $sold_Count = $datas['sold_count'];
            $check_status = $datas['check_status'];
         

            
            //  echo "<pre>";
            // print_r($fee_countN);
             $conn->query("update fees_master set sold_count= '$fee_countN' where id=$parent_id");
            setAnswer($conn,$no_of_tickets,$pPool,$created_date,$modify_date, $fee_count,$userId, $pkg_id,$price,$question,$answer_key,$no_of_winners,$status,$tbl_packages_status,$parent_id,$count,$check_status,$sold_Count);
        }
      // break;
    //}
...
function setAnswer($conn,$no_of_tickets,$pPool,$created_date,$modify_date, $fee_count,$userId, $pkg_id,$price,$question,$answer_key,$no_of_winners,$status,$tbl_packages_status,$parent_id,$count,$check_status,$sold_Count){
     
    
    if((int)$no_of_tickets == (int)$sold_Count && $check_status ==0 ){
        //print_r($userId);
        $conn->query("update fees_master set check_status =2 where id=$parent_id");
    for($i=0; $i<1; $i++){
           ...
           // echo $last_insert_id;
           // $conn->query("update fees_master set check_status ==2 where id=$last_insert_id");
